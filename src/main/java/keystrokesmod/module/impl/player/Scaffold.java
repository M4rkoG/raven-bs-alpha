...
public class Scaffold extends Module {
    ...
    public void onPreMotion(PreMotionEvent e) {
        ...
        if (Utils.keysDown() && usingFastScaffold() && fastScaffold.getInput() >= 1 && !ModuleManager.tower.canTower() && !blockAbove()) { // jump mode
        ...
        //Float
        if (sprint.getInput() == 2 && !usingFastScaffold() && !ModuleManager.tower.canTower()) {
    ...
    public String getInfo() {
        return Mouse.isButtonDown(1) && fastOnRMB.isToggled() ? fastScaffoldModes[handleFastScaffolds()] : sprintModes[handleFastScaffolds()];
    }
    ...
    private int handleFastScaffolds() {
        if (fastOnRMB.isToggled()) {
            return Mouse.isButtonDown(1) && (mc.currentScreen == null && mc.inGameHasFocus) ? (int) fastScaffold.getInput() : (int) sprint.getInput();
        }
        else {
    ...
    private boolean usingFastScaffold() {
        return fastScaffold.getInput() > 0 && (!fastOnRMB.isToggled() || Mouse.isButtonDown(1) && (mc.currentScreen == null && mc.inGameHasFocus));
    }
