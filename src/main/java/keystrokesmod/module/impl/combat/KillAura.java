...
public class KillAura extends Module {
    ...
    private int switchFirst;
    private int partialTicks;

    // blink related
    private ConcurrentLinkedQueue<Packet> blinkedPackets = new ConcurrentLinkedQueue<>();
    private ConcurrentLinkedQueue<Packet> abPackets = new ConcurrentLinkedQueue<>();
    private AtomicBoolean blinking = new AtomicBoolean(false);
    private boolean lag;
    private boolean swapped;
    private boolean didAutoblock;
    ...
    public void onSendPacket(SendPacketEvent e) {
        ...
        if (blinking.get() && !e.isCanceled()) { // blink
            ...
            abPackets.add(packet);
            if (packet instanceof C09PacketHeldItemChange || packet instanceof C08PacketPlayerBlockPlacement || packet instanceof C02PacketUseEntity || packet instanceof C07PacketPlayerDigging) {
                //Nothing
            }
            else {
                    ...
                    if (switchFirst == 0) {
                        switch (interactTicks) {
                            ...
                            case 2:
                                ...
                                lag = true;
                                didAutoblock = true;
                                //Utils.print("Tick 2 Cycle 1");
                                break;
                            case 3:
                                firstCycle = true;
                                //Utils.print("Tick 3 Cycle 1");
                                switchFirst = 1;
                                break;
                        }
                    } else if (switchFirst == 1) {
                        switch (interactTicks) {
                            case 1:
                                break;
                            case 2:
    ...
    public void resetBlinkState(boolean unblock) {
        if (didAutoblock && !Utils.holdingSword() && mc.thePlayer.inventory.currentItem != getBestSwapSlot() ) {
        ...
        lag = swapped =  didAutoblock = false;
        hasBlocked = false;
    }

    private void releasePackets() {
        try {
            synchronized (abPackets) {
                for (Packet packet : abPackets) {
            ...
            e.printStackTrace();
            Utils.sendModuleMessage(this, "&cThere was an error releasing ab packets");
        }
        abPackets.clear();
        try {
            synchronized (blinkedPackets) {
                for (Packet packet : blinkedPackets) {
                    Raven.packetsHandler.handlePacket(packet);
                    PacketUtils.sendPacketNoEvent(packet);
                    //Utils.print("Sent regular packets");
                }
            }
        }
        catch (Exception e) {
